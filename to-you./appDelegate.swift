...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    // Shared model powering UI + menubar
    let model = AppModel()
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Observe ticks to update menubar title/icon
        model.onTick = { [weak self] left in
            ...
            if self.model.isRunning {
                ...
                // Use the formatted method we re-exposed
        ...
        // Popover content
        let content = TimerPopoverView(model: model,
...
//
//        // Popover content
//        let content = TimerPopoverView(model: model, showHUD: { [weak self] in self?.showHUD() }, hideHUD: { [weak self] in self?.hud.hide() })
//        popover.behavior = .transient
//        popover.contentSize = NSSize(width: 300, height: 240)
//        popover.contentViewController = NSViewController()
//        popover.contentViewController?.view = NSHostingView(rootView: content)
...
//            popover.show(relativeTo: button.bounds, of: button, preferredEdge: .minY)
//        }
////        SoundManager.shared.playNamed("SoftChime")
//    }
//}
